...
class Post:
    ...
    def make_reaction(self, reaction_id: int) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(f"{MAIN_URL}/posts/{self.id}/react?reaction_id={reaction_id}").content, 'lxml').find('input', {'name': '_xfToken'})['value']
    ...
    def edit(self, html_message: str):
        ...
        token = BeautifulSoup(self.API.session.get(MAIN_URL + f"/posts/{self.id}/edit").content, 'lxml').find('input', {'name': '_xfToken'})['value']
    ...
    def delete(self, reason: str, hard_delete: int = 0) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(MAIN_URL + f"/posts/{self.id}/delete").content, 'lxml').find('input', {'name': '_xfToken'})['value']
        self.API.session.post(f"{MAIN_URL}/posts/{self.id}/delete", {"reason": reason, "hard_delete": hard_delete, "_xfToken": token})
        return True


class ProfilePost:
    def __init__(self, API: 'ArizonaAPI', id: int, creator: 'Member', profile: 'Member', create_date: int, bb_content: str) -> None:
        self.API = API
        self.id = id
        self.creator = creator
    ...
    def make_reaction(self, reaction_id: int) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(f"{MAIN_URL}/profile-posts/{self.id}/react?reaction_id={reaction_id}").content, 'lxml').find('input', {'name': '_xfToken'})['value']
    ...
    def comment(self, message_html: str) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(MAIN_URL + f"/profile-posts/{self.id}/add-comment").content, 'lxml').find('input', {'name': '_xfToken'})['value']
    ...
    def delete(self, reason: str, hard_delete: int = 0) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(MAIN_URL + f"/profile-posts/{self.id}/delete").content, 'lxml').find('input', {'name': '_xfToken'})['value']
    ...
    def edit(self, html_message: str):
        ...
        token = BeautifulSoup(self.API.session.get(MAIN_URL + f"/profile-posts/{self.id}/edit").content, 'lxml').find('input', {'name': '_xfToken'})['value']
