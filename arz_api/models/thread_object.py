...
class Thread:
    def __init__(self, API: 'ArizonaAPI', id: int, creator: 'Member', category: 'Category', create_date: int, title: str, content: str, html_content: str, pages_content: int, thread_post_id: int, is_closed: bool) -> None:
        self.API = API
        self.id = id
    ...
    def answer(self, html_message: str) -> bool:
        ...
        self.API.session.post(f"{MAIN_URL}/threads/{self.id}/add-reply", {'_xfToken': token, 'message_html': html_message})
        return True
    ...
    def watch(self, stop: bool, email_subscribe: bool = False) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(f"{MAIN_URL}/help/terms/").content, 'lxml').find('html')['data-csrf']
        self.API.session.post(f"{MAIN_URL}/threads/{self.id}/watch", {'_xfToken': token, 'stop': int(stop), 'email_subscribe': int(email_subscribe)})
        return True
    ...
    def close(self) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(f"{MAIN_URL}/help/terms/").content, 'lxml').find('html')['data-csrf']
        self.API.session.post(f"{MAIN_URL}/threads/{self.id}/quick-close", {'_xfToken': token})
        return True
    ...
    def pin(self) -> bool:
        ...
        token = BeautifulSoup(self.API.session.get(f"{MAIN_URL}/help/terms/").content, 'lxml').find('html')['data-csrf']
        self.API.session.post(f"{MAIN_URL}/threads/{self.id}/quick-stick", {'_xfToken': token})
        return True
    

    def edit(self, html_message: str) -> bool:
    ...
    def edit_info(self, title: str = None, prefix_id: int = None) -> bool:
        ...
        self.API.session.post(f"{MAIN_URL}/threads/{self.id}/edit", data)
        return True
